...
export const meta: Meta = {
  description: "Retrieve the histrogram data needed to render a histogram.",
  details: `Notes: 
- Perform poorly / inaccurate binning when values are very close to bin boundaries.
- Binning uses [min, max)[min, max)...[min,max)[min,max] strategy
- Therefore, max values are inserted to last bin.`,
  examples: [
    {
      name: "Getting Histogram Data",
      code: console => {
        console.log(get_histogram_data({
          samples: [
    ...
    {
      name: "Inaccuracies regarding values being too close to bin boundaries",
      code: (console) => {
        console.log(get_histogram_data({
          samples: [
